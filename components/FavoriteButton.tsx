...
const FavoriteButton: React.FC<FavoriteButtonProps> = ({ movieId }) => {
  ...
  const toggleUseFavorite = useCallback(async () => {
    let response;

    // if current movie is already in favorites then delete the current attempt to add it, leaving the already available one
    //else add it
    if (isFavorite) {
